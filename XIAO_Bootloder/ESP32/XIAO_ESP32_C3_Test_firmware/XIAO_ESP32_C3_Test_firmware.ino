...
// 定义输出引脚数组（与输入引脚一一对应）
const uint8_t OUTPUT_PINS[] = {D2, D3, D5};  
const uint8_t Button = D9;
    ...
    TestStatus saved_status = read_test_result_from_nvs();
        ...
        switch(saved_status) {
            case TEST_PASSED:
                ...
                Serial.println("Hello from Seeed Studio XIAO ESP32-C3");
                break;
            case TEST_FAILED:
...
int testGPIO() {
  ...
  for (int i = 0; i < sizeof(OUTPUT_PINS) / sizeof(OUTPUT_PINS[0]); i++) {
    ...
    // MySerial0.print(" with OUTPUT Pin ");
    // MySerial0.println(OUTPUT_PINS[i]);
    delay(500);                          // 等待0.5秒
  
    // 检查对应输入引脚的状态
    for (int j = 0; j < sizeof(INPUT_PINS) / sizeof(INPUT_PINS[0]); j++) {
      int state = digitalRead(INPUT_PINS[j]);
      // MySerial0.print("Checking INPUT Pin ");
      // MySerial0.println(INPUT_PINS[j]);
      // MySerial0.print(": ");
      if (i == j) {  // 如果是当前对应的输入引脚
        MySerial0.println(state == HIGH ? "HIGH (PASS)" : "LOW (FAIL)");
        if (state != HIGH) {
          count++;
        }
      } else if (INPUT_PINS[j] == D4 ) {  // D1特殊情况，始终接地
        MySerial0.println(state == LOW ? "LOW (Expected)" : "HIGH (Unexpected)");
        if (state != LOW) {
          count++;
        }
      } else {  // 其他非对应引脚
        MySerial0.println(state == LOW ? "LOW (PASS)" : "HIGH (FAIL)");
        if (state != LOW) {
